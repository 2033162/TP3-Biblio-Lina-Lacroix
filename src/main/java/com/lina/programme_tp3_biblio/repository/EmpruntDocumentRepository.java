...
public interface EmpruntDocumentRepository extends JpaRepository<EmpruntDocuments, Long> {
    ...
    @Query(value = "SELECT * from Emprunt_Documents e WHERE e.client = :clientId AND e.date_Expire < CURRENT_DATE()", nativeQuery = true)
    List<EmpruntDocuments> getClientEmpruntRetard(@Param("clientId") long clientId);

    @Query(value = "SELECT count(*) AS nbrEmprunt FROM Emprunt_Documents e WHERE e.document = :empruntDocumentsId", nativeQuery = true)
